	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			if (inputMenu == 1) {
				...
				if (stocks[inputCode] < saftyStocks[inputCode]) {

					if (balance > saftyStocks[inputCode] * 2 * costs[inputCode]) {
						stocks[inputCode] += saftyStocks[inputCode] * 2;
						balance -= saftyStocks[inputCode] * 2 * costs[inputCode];
						// 지출등록
						expences += saftyStocks[inputCode] * 2 * costs[inputCode];
